...
export default function MetiersCM1() {
  ...
  useEffect(() => {
    getBestScore("cm1", "anglais", "metiers").then(setBestScore);
    getLastScore("cm1", "anglais", "metiers").then(setLastScore);
  }, []);
  ...
  const handleSuivant = async (currentScore: number) => {
    if (qIndex + 1 >= questionsActives.length) {
      ...
      const [best, last] = await Promise.all([
        getBestScore("cm1", "anglais", "metiers"),
        getLastScore("cm1", "anglais", "metiers"),
      ]);
      setBestScore(best);
      setLastScore(last);
      setEtape("fini");
